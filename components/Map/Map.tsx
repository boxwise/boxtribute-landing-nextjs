...
const Map = ({ center, baseMarkers }: IMapContainerData) => {
  ...
  const getMarkerIcon = (status?: "active" | "inactive") => {
    const color = status === "inactive" ? "#964B00" : "#EF4049";
    return L.divIcon({
      html: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="${color}" width="36" height="36" aria-hidden="true">
        <path fill-rule="evenodd" d="m11.54 22.351.07.04.028.016a.76.76 0 0 0 .723 0l.028-.015.071-.041a16.975 16.975 0 0 0 1.144-.742 19.58 19.58 0 0 0 2.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827[...]
        </svg>`,
      className: "",
      iconSize: [36, 36],
      iconAnchor: [18, 36],
  ...
  return (
    ...
    >
      ...
      {baseMarkers.map((marker, i) => (
        <Marker key={i} icon={getMarkerIcon(maker.status)} position={[marker.position.lat, marker.position.long]}>
          <Popup
